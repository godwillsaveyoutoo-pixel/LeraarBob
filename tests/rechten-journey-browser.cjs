...
(async()=>{
 ...
 await navigate();
 async function tap(selector){const p=await c.eval(`(()=>{const b=document.querySelector(${JSON.stringify(selector)});if(!b||b.disabled)throw Error('Cannot tap '+${JSON.stringify(selector)});const r=b.getBoundingClientRect();return {x:r.x+r.width/2,y:r.y+r.height/2}})()`);await c.send('Input.dispatchTouchEvent',{type:'touchStart',touchPoints:[{...p,radiusX:3,radiusY:3}]});await c.send('Input.dispatchTouchEvent',{type:'touchEnd',touchPoints:[]});await c.eval('new Promise(r=>requestAnimationFrame(()=>requestAnimationFrame(r)))')}
 const reset=async(ready=false)=>c.run(`stopAccountExercise();state=DEFAULT();guestMode=true;currentScreen='play';${ready?`for(const k of J.skills){const s=state.skills[k];s.intro=true;s.seen=8;s.correct=8;s.strength=.85;s.recent=[true,true,true,true]}state.access=[...J.skills];`:''}save();openScreen('journey')`);
 async function layout(where){const bad=await c.eval(`(()=>{const bad=[];for(const e of document.querySelectorAll(${JSON.stringify(where==='map'?'#journeyPanel button,#journeyPanel h2,#journeyPanel h3,.journey-status,.journey-footer,.topbar':'#stage button,#question,#status,.topbar')})){if(!e.getClientRects().length)continue;const r=e.getBoundingClientRect();if(r.left<-.5||r.right>innerWidth+.5||r.top<0||r.bottom>innerHeight+.5)bad.push(e.textContent+' outside '+JSON.stringify(r.toJSON()));if(e.tagName==='BUTTON'&&(r.width<47.5||r.height<47.5))bad.push('small '+e.textContent)}if(document.documentElement.scrollHeight>innerHeight||document.documentElement.scrollWidth>innerWidth)bad.push('page overflow');return bad})()`);assert.deepEqual(bad,[],where)}
