 ...
 try{
  ...
  for(const [index,zone] of ['bouwen','omzetten'].entries()){
   ...
   for(const id of expected[index]){
    if(index===0||expected[1].slice(0,3).includes(id)){await c.tap(`[data-node="${id}"]`,true);assert.equal(await c.eval('document.querySelector("#app").dataset.screen'),'mission');await c.tap('#pause',true);assert.equal(await c.eval('location.hash'),'#formulewerf/'+zone);continue}
