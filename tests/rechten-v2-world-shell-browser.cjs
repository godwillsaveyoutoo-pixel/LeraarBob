...
(async()=>{
 ...
 try{
  ...
  const rootChoice=async()=>{const t=(await snap()).missions.positive.task;return String(t.options.findIndex(q=>q.n===t.root.n&&q.d===t.root.d))};
  const answer=await rootChoice();await choose('answer',answer);await layout('root chosen');await resume('partial root');
  await c.tap('#pause',true);assert.equal(await screen(),'area');assert.equal((await snap()).missions.positive.values.answer,answer);
  await c.eval('history.back()');await c.wait('document.querySelector("#app").dataset.screen==="mission"');assert.equal((await snap()).missions.positive.values.answer,answer);report.checks.push('browser Back resumes exact task and draft');
  await c.tap('#commit',true);assert((await snap()).missions.positive.feedback.result.ok);await resume('committed root');await c.tap('#continue',true);
  await choose('inequality','>');await c.tap('#commit',true);assert.equal((await snap()).missions.positive.feedback.result.code,'sign.side_reversed');await layout('incorrect inequality');await c.shot('interval-repair-640x360');await resume('incorrect inequality');await c.tap('#continue',true);
  assert.equal((await snap()).missions.positive.values.answer,answer);assert.equal((await snap()).missions.positive.locks.answer,true);await choose('inequality','<');await c.tap('#commit',true);assert((await snap()).missions.positive.feedback.result.ok);await layout('correct inequality');await c.shot('interval-correct-640x360');await c.tap('#continue',true);
  assert.equal((await snap()).missions.positive.index,1);
  while(!(await snap()).missions.positive.completed){const m=(await snap()).missions.positive;await choose(m.phase==='grens-root'?'answer':'inequality',m.phase==='grens-root'?await rootChoice():m.task.model.a.n>0?'>':'<');await c.tap('#commit',true);assert((await snap()).missions.positive.feedback.result.ok);await c.tap('#continue',true)}
  await c.tap('.boundary-footer [data-screen=area]',true);assert.equal(await screen(),'area');assert.equal(await c.eval('document.querySelector("[data-node=positive]").dataset.state'),'completed');await resume('completed area');await c.shot('area-completed-640x360');
  assert((await snap()).events.every(e=>e.mastery===false));report.checks.push('world → area → full positive sequence → completed area; repair and idempotent reload');
  await c.tap('[data-node=positive]',true);assert.equal((await snap()).missions.positive.run,2);await c.tap('#pause',true);assert.equal(await c.eval('document.querySelector("[data-node=positive]").dataset.state'),'completed','completed badge survives explicit replay');
  ...
  await c.viewport(1366,768,false);await fresh();c.keyboardOnly=true;await c.tap('[data-world-node="grenspas"]');await c.tap('[data-node=positive]');await choose('answer',await rootChoice(),false);await c.tap('#commit');await c.tap('#continue');await choose('inequality','<',false);await c.tap('#commit');await c.shot('exercise-correct-1366x768');await c.tap('#continue');assert.equal((await snap()).missions.positive.index,1);await c.tap('#pause');assert.equal(await screen(),'area');c.keyboardOnly=false;report.checks.push('complete positive task by keyboard without pointer input');
