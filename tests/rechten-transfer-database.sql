...
begin
 ...
 wave:=jsonb_set(wave,'{version}','703');
 foreach k in array array['rewrite_linear_equation','input_from_output','point_on_line'] loop
  wave:=jsonb_set(wave,array['skills',k],'{"strength":0,"seen":0,"correct":0}');
 end loop;
 result:=pg_temp.wave_save_progress(wave,3);
 if result->>'status'<>'saved' or result->>'revision'<>'4' then raise exception 'FAIL v703 upgrade'; end if;
 foreach k in array array['700','701','702'] loop
  denied:=false;
  begin perform pg_temp.wave_save_progress(jsonb_set(wave,'{version}',to_jsonb(k::int)),4); exception when invalid_parameter_value then denied:=true; end;
  if not denied then raise exception 'FAIL older client overwrite'; end if;
 end loop;
 foreach k in array array['rewrite_linear_equation','input_from_output','point_on_line'] loop
  denied:=false;
  begin perform pg_temp.wave_save_progress(wave #- array['skills',k],4); exception when invalid_parameter_value then denied:=true; end;
  if not denied then raise exception 'FAIL missing algebra skill'; end if;
 end loop;
 result:=pg_temp.wave_save_progress(wave,3);
 if result->>'status'<>'conflict' then raise exception 'FAIL v703 conflict'; end if;
 wave:=jsonb_set(wave,'{version}','704');
 foreach k in array array['graph_from_table','equation_from_graph','equation_from_table','equation_from_context'] loop
  wave:=jsonb_set(wave,array['skills',k],'{"strength":0,"seen":0,"correct":0}');
 end loop;
 result:=pg_temp.wave_save_progress(wave,4);
 if result->>'status'<>'saved' or result->>'revision'<>'5' then raise exception 'FAIL v704 upgrade'; end if;
 foreach k in array array['700','701','702','703'] loop
  denied:=false;
  begin perform pg_temp.wave_save_progress(jsonb_set(wave,'{version}',to_jsonb(k::int)),5); exception when invalid_parameter_value then denied:=true; end;
  if not denied then raise exception 'FAIL older client overwrite'; end if;
 end loop;
 foreach k in array array['graph_from_table','equation_from_graph','equation_from_table','equation_from_context'] loop
  denied:=false;
  begin perform pg_temp.wave_save_progress(wave #- array['skills',k],5); exception when invalid_parameter_value then denied:=true; end;
  if not denied then raise exception 'FAIL missing transfer skill'; end if;
 end loop;
 result:=pg_temp.wave_save_progress(wave,4);
 if result->>'status'<>'conflict' then raise exception 'FAIL v704 conflict'; end if;
...
$$;
select 'PASS: v700, v701, v702, v703, v704, transfer/algebra/construction skills, counters, all skills, revisions, stale clients and account binding; temporary fixtures only' as result;
