...
const wave=process.argv.includes('--wave3')?'wave3':process.argv.includes('--wave2')?'wave2':'wave1';
...
const tests=fs.readFileSync(path.join(root,wave==='wave3'?'tests/rechten-algebra-database.sql':wave==='wave2'?'tests/rechten-construction-database.sql':'tests/rechten-wave-database.sql'),'utf8'),split=tests.indexOf('do $$');
