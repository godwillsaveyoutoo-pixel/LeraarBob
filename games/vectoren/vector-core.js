...
const skills=[
 ...
 skill('decompose','Ontbinden in richtingen',['sum'],'Zoek twee componenten in de gegeven richtingen. Hun som moet r zijn. In deze opgaven zijn beide componenten niet nul; ze mogen ook de tegengestelde zin hebben.'),
...
function generate(id,{seed=1,level=0,variant=0,repair=null}={}){
 ...
 if(id==='props'){
  ...
  // Choose nonzero coefficients in the given basis, including for oblique directions.
  const k=pick([1,2,3])*sign(),l=pick([1,2])*sign();
  t.dirs=level&&v%2?[vec(1,1),vec(-1,1)]:[vec(1,0),vec(0,1)];t.target=add(scale(t.dirs[0],k),scale(t.dirs[1],l));t.policy='decompose';t.refs=[ref(P,t.target,'r')];t.prompt='Ontbind r volgens de twee stippelrichtingen. Teken zelf beide componenten.';
 }else if(id==='coords'){
  // Drawing both components requires a displacement along both axes.
  t.target=level===0?vec(pick([1,2,3])*sign(),pick([1,2])*sign()):a;
  t.refs=[ref(P,t.target,'a')];t.points=[];t.prompt='Bepaal de vectorcoördinaten van a.';
...
function freshState(){return {version:2,total:0,xp:0,sessions:0,activity:[],skills:Object.fromEntries(skills.map(s=>[s.id,freshSkill()])),repairs:[],lastSkill:null,lastSignatures:[]}}
function sanitize(raw){const p=freshState();if(!raw||raw.version!==2)return p;
 ...
 p.activity=sanitizeActivity(raw.activity);
