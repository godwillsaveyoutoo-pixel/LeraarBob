...
function label(e){switch(e.kind){case 'fraction':return `${format(e.n)}/${e.d}`;case 'integer':return format(e.n);case 'decimal':return e.text.replace('-','−');case 'percent':return `${format(e.n)}%`;case 'root':return `${e.sign<0?'−':''}${e.degree===3?'∛':'√'}${e.n<0?'('+format(e.n)+')':e.n}`;case 'pi':return 'π';case 'period':return `${e.whole||0},${e.lead||''}(${e.repeat}) periodiek`;default:return ''}}
const skills=[
...
function generate(skill,{seed=1,variant=0,level=0,contentVersion=2}={}){
...
function freshAnswer(t){return {values:t.skill==='interval'?['','']:['',''],sign:1,relation:null,groups:Array(6).fill(null),labels:[],tick:null,closedLo:false,closedHi:false,start:null,end:null,stage:0,periodAnchor:null,placements:Array(t.tokens?.length||4).fill(null),decimalType:null}}
...
function validate(t,a){
 ...
 if(t.skill==='root'){
  ...
  const [lower,upper]=rootBounds(t,a.stage);
