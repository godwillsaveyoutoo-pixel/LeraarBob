...
function mount(points,w,stage,{visual,answers,question,status,footer,redraw,submit,undo}){
 ...
 const slot=(r,i)=>{const a=r==='ys'?'y':'x',point=r===stage?active[i]:w.values[r]?.[i];return `<button type="button" class="coordinate-slot ${point?'filled':''}" data-coord-slot="${i}" data-coord-row="${r}" ${r!==stage?'disabled':''} aria-label="${r==='ys'?'Teller':'Noemer'}, ${i===0?'eerste':'tweede'} coördinaat${point?', '+point+': '+C.text(points[point][a]):', leeg'}">${point?`<span class="coordinate-value">${value(point,a)}</span>`:`<span class="coordinate-empty" aria-hidden="true"></span>`}</button>`};
 visual.innerHTML=`<div class="coordinate-builder"><div class="coordinate-points" aria-label="Gegeven punten">${['A','B'].map(point=>`<div class="coordinate-point"><strong>${point}</strong><span>(</span>${['x','y'].map(a=>`<button type="button" class="coordinate-token ${chosen?.point===point&&chosen.axis===a?'selected':''}" data-coord-point="${point}" data-coord-axis="${a}" aria-label="${point}, ${a} = ${C.text(points[point][a])}" aria-pressed="${chosen?.point===point&&chosen.axis===a}"><span class="coordinate-value">${C.html(points[point][a])}</span></button>`).join('<span>;</span>')}<span>)</span></div>`).join('')}</div><p class="coordinate-instruction">Sleep of klik de coördinaten.</p><div class="coordinate-equation"><span class="coordinate-a">a =</span><div class="coordinate-fraction"><div class="coordinate-row ${stage==='ys'?'active':''}"><span class="coordinate-label">Δy</span>${slot('ys',0)}<span>−</span>${slot('ys',1)}</div><div class="coordinate-row ${stage==='xs'?'active':''}"><span class="coordinate-label">Δx</span>${slot('xs',0)}<span>−</span>${slot('xs',1)}</div></div></div><p class="coordinate-direction">${w.values.ys?`${w.values.ys.join(' − ')} boven én onder.`:''}</p></div>`;
