...
function mount(t,opts){
 ...
 visual.classList.add('algebra-visual');question.textContent=w.done?'Controle afgerond':labels[stage];answers.className='answers wave-controls algebra-controls';answers.replaceChildren();footer.replaceChildren();
 ...
 const submit=value=>{const r=C.submit(t,w,value);status.className='status '+(r.ok?'good':'bad');status.textContent=r.ok?r.message||'Stap klopt.':r.message;if(w.done){onDone();status.textContent=w.errors.length||w.help?'Gecontroleerd na herstel of hulp.':'Zelfstandig gecontroleerd.'}redraw()};
 let givens=t.skill==='rewrite_linear_equation'?C.equationHTML(p.equation):C.formula(p.model.a,p.model.b).replace('y =','f(x) =');
 if(t.skill==='point_on_line')givens+=' · '+pHTML(p.P);if(t.skill==='input_from_output')givens+=`<small>Gegeven uitvoer: ${H(p.target)}</small>`;
 let line='';
 if(stage==='algebra'||stage==='modelKind')line=C.equationHTML(C.algebraEquation(t,w));
 if(stage==='subPoint')line='f(□) = a · □ + b';
 if(stage==='subOutput')line=`□ = ${H(p.model.a)}x + (${H(p.model.b)})`;
 ...
 visual.innerHTML=`<div class="wave-context"><div class="wave-givens">${givens}</div><div class="wave-equation">${line}</div>${extra}</div>`;
 ...
 if(numeric){
  answers.classList.add('wave-keypad');const el=document.createElement('div');el.className='wave-entry';const op=w.operation,verb=op?(op.kind==='divide'?'Delen door':op.kind==='subtract'?'Aftrekken:':'Optellen:'):'Uitkomst:';el.innerHTML=`<small>${verb}${op&&op.kind!=='divide'?' '+(op.term==='c'?'getal':op.term+'-term'):''}</small><span class="frac"><span class="${w.part===0?'active':''}">${w.entry[0]||'□'}</span><span class="${w.part===1?'active':''}">${w.entry[1]||'□'}</span></span>`;visual.querySelector('.wave-context').append(el);
  ...
  button(op?'Pas toe':'Controleer',()=>{const value=C.parse(w.entry.join('/'));if(!value){status.textContent='Vul een getal in met een noemer ongelijk aan nul.';return}submit(op?{...op,value}:value)},footer);
 }else if(stage==='algebra'){
  if(w.operation){question.textContent='Op welke term werkt je bewerking?';for(const [term,label] of [['x','x-term'],['y','y-term'],['c','getal']])button(label,()=>{w.operation.term=term;redraw()})}
  else {for(const [kind,label] of [['add','+ op beide leden'],['subtract','− op beide leden'],['divide','÷ beide leden']])button(label,()=>{w.operation={kind,term:kind==='divide'?'c':null};w.entry=['','1'];w.part=0;redraw()});button('Klaar met herleiden',()=>submit({kind:'finish'}),footer)}
 }else {const choices={subPoint:[['x','x = '+H(p.P?.x||C.q(0))],['y','y = '+H(p.P?.y||C.q(0))]],subOutput:[['output','f(x) = '+H(p.target||C.q(0))],['input','x = '+H(p.target||C.q(0))]],pointVerdict:[['on','ligt erop'],['off','ligt ernaast']],modelKind:[['function','functie y = f(x)'],['vertical','verticale rechte']],constantSolutions:[['all','alle x ∈ ℝ'],['none','geen x'],['one','één x']]};for(const [value,label] of choices[stage])button(label,()=>submit(value))}
