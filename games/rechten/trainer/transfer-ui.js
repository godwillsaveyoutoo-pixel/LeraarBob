...
function explanation(skill){const data={graph_from_table:['Tabel → punten → rechte → controle','Kies twee kolommen en plaats hun punten op het rooster. Trek de rechte en controleer daarna het derde tabelpunt. Past dat niet, dan gaat er geen enkele rechte door alle drie.'],equation_from_graph:['Kies zelf twee punten op de rechte','Lees de assenschaal, kies twee verschillende roosterpunten en bereken a. Lees b af als het y-snijpunt zichtbaar is, of bereken b met één van je punten. Maak de formule en controleer je punten.'],equation_from_table:['Twee kolommen → formule → derde kolom','Kies twee verschillende kolommen, bereken a en b en maak je formule. Controleer ook de overblijvende kolom: de tabel kan inconsistent zijn. Dan is er geen passend affine voorschrift.'],equation_from_context:['Grootheden → a en b → voorschrift','De verandering per eenheid is a; de startwaarde is b. Afname geeft een negatieve a. Later gebruik je twee beschreven situaties. Controleer je formule en het geldige bereik, met eenheden.']};const [big,sub]=data[skill];return {title:C.catalog[skill].label,big,sub,visual:'<div class="construction-example">Gegevens → model<small>Kies · bereken · controleer</small></div>'}}
function mount(t,opts){
 ...
 const redraw=()=>{onChange();mount(t,opts)},submit=value=>{const r=C.submit(t,w,value);status.className='status '+(r.ok?'good':'bad');status.textContent=r.ok?'Stap klopt.':r.message;if(w.done){onDone();status.textContent=w.errors.length||w.help?'Gecontroleerd na herstel of hulp.':'Zelfstandig gecontroleerd.'}redraw()};
 ...
 if(['colA','colB'].includes(stage))table(t,w,submit,button,answers);
  ...
  svg.onpointercancel=()=>{pending=null;preview(w.cursor||{x:0,y:0})};svg.onpointerup=e=>{if(!pending)return;gridGesture=true;if(svg.hasPointerCapture(e.pointerId))svg.releasePointerCapture(e.pointerId);remember();w.cursor=pending;pending=null;onChange();requestAnimationFrame(()=>{if(svg.isConnected)mount(t,opts)})};
