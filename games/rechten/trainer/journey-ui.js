...
window.RechtenJourneyUI={render(root,{state,unlocked,ready,phase,order,requirements,labels,busy,activity='learn',onActivity,onPrepare,onSelect,onRegion,onAtlas,onRecommend,onStart,onResume,onStop}){
 ...
 const details=`<aside class="journey-details" aria-label="Geselecteerde stop"><div class="journey-detail-main"><span class="journey-context">${inspect==='challenge'?'De eindhalte van dit hoofdstuk':sameRound?'Verder waar je was':recommended?`Ronde ${round} · jouw volgende stop`:'Deze stop bekijken'}</span><h3 tabindex="-1">${focus}</h3><p>${intro}</p>${!inspect&&view==='area'&&recommended&&!sameRound&&rec.skill?`<p class="journey-focus">Deze ronde: <strong>${skillLabel(rec.skill)}</strong></p>`:''}${!inspect&&!s.open&&!sameRound?`<p class="journey-preparation">Bereid eerst ${missing.map(k=>labels[k]?.label||k).join(' en ')||'de eerdere leerdoelen'} voor.</p>`:''}${inspect==='challenge'?`<div class="journey-test-progress"><span>${J.skills.length-needs.length} van 5 leerdoelen voorbereid</span><span>${proof.length} van 5 zelfstandig aangetoond</span></div>${needs.length?`<p class="journey-preparation">Oefen nog: ${needs.map(skillLabel).join(', ')}.</p>`:''}`:''}${sameRound?`<div class="journey-round-progress"><span>Ronde ${round}<strong>${answered} / 12</strong></span><progress max="12" value="${answered}" aria-label="Beantwoorde opgaven in de lopende ronde"></progress></div>`:''}${action}<p class="journey-small">${sameRound?'Je gaat verder met je bewaarde opgave.':inspect==='challenge'?'12 opgaven · 5 leerdoelen':'12 opgaven · op jouw tempo'}</p>${resume&&!sameRound?'<p class="journey-small">Als je hier start, stopt de huidige ronde. Je behaalde resultaten blijven bewaard.</p>':''}${!inspect&&view==='atlas'?'<button class="journey-text-button" data-recommend>Bekijk de stopplaatsen →</button>':!inspect&&!recommended&&s.open&&!resume?'<button class="journey-text-button" data-recommend>Terug naar mijn volgende stop →</button>':''}</div><div class="journey-detail-extra">${result}<p class="journey-status">${inspect==='challenge'?(proof.length===5?'✓ Alle vijf leerdoelen zelfstandig aangetoond':needs.length?'Nog voorbereiding nodig':'Klaar voor een toetspoging'):stateLabel(s)}${!inspect&&s.completed&&s.review?' · ✓ Ronde afgerond':''}</p><details class="journey-goals"><summary>${inspect==='challenge'?'Leerdoelen in deze toets':'Vaardigheden en voorbereiding'}</summary><ul>${focusSkills}</ul></details></div></aside>`;
 ...
 const header=`<div class="journey-heading"><div><span>${view==='atlas'?`JOUW LEERREIS · HOOFDSTUK ${chapter} VAN 5`:'RECHTEN / JOUW LEERREIS'}</span><h2 tabindex="-1">${view==='atlas'?'Elke stop een stap verder.':r.name}</h2></div>${view==='area'?'<button data-atlas>← Alle hoofdstukken</button>':`<div class="journey-total"><span><strong>${completed}</strong> / ${J.places.length} afgerond</span><span>${icon('star')}<strong>${mastered}</strong> beheerst</span><progress max="${J.places.length}" value="${completed}" aria-label="Afgeronde stopplaatsen"></progress></div>`}</div>`;
