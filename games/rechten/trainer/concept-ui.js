...
function context(t,w,stage,graph){
 ...
 if(!direction){
  ...
  if(w.done)note=m.kind==='vertical'?`Bij x = ${H(m.c)} horen meerdere y-waarden. Geen functie y = f(x).`:m.kind==='identical'?'Door één punt passen meerdere rechten. Er is geen unieke rechte.':`Bij elke x hoort dezelfde y = ${H(m.b)}. Dit is een functie; a = 0.`;
 ...
 return `<div class="concept-context ${showGraph?'with-graph':'concept-data'}"><div class="concept-givens">${givens}</div>${showGraph?`<div class="concept-graph">${graph(p,{alternatives:revealed&&m.kind==='identical'})}</div>`:''}${formula?`<div class="concept-formula">${formula}</div>`:''}${note?`<p class="concept-note">${note}</p>`:''}</div>`;
}
