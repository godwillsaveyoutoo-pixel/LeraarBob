...
function mount(t,{visual,answers,question,status,footer,onChange,onDone,onNext,onResult,onRendered,dev=false}){
 ...
 if(['ys','xs','a'].includes(stage)){root.RechtenCoordinateBuilder.mount(t.params,w,stage,{visual,answers,question,status,footer,redraw,submit,undo:()=>C.undo(w),guided:!!t.coordinateGuidance,title:t.skill==='slope_from_two_points'?slopeTitle:'Bepaal eerst a.'});if(pointTask(t))pointQuestion(t,w,stage,question);return}
 ...
 button('Terug',()=>{C.undo(w);redraw()},footer,'back').disabled=!w.history.length;
}
