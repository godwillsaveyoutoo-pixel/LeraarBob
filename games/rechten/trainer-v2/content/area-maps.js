...
const areas={
 ...
 formulewerf:{name:'Formulewerf',caption:'Voorschriften en representaties',intro:'Van de basisvorm naar zelf afleiden.',art:'formulewerf',zones:[{id:'bouwen',badge:'A',name:'Voorschrift en grafiek',intro:'Gebruik en herken de basisvorm y = ax + b. Daarna leid je zelf voorschriften af in werkplaats B.',nodes:[
  node('equation_from_ab','Voorschrift uit a en b','Voorschrift<br>uit a en b',24,25,{playable:true}),
  node('graph_from_equation','Rechte uit voorschrift','Rechte uit<br>voorschrift',76,25,{playable:true}),
  node('equation_from_graph','Voorschrift uit grafiek','Voorschrift<br>uit grafiek',68,62,{playable:true,entryPolicy:Object.freeze({difficultyLayers:Object.freeze([0,1]),readableSlope:true,visibleIntercept:true,requiresInterceptFromArbitraryPoint:false,requiresExtendedTwoPointCalculation:false})}),
  node('rewrite_linear_equation','Schrijf de vergelijking in de vorm y = ax + b','Schrijf de vergelijking<br>in de vorm<br>y = ax + b',32,58,{playable:true})
  ...
  node('intercept_from_point','b uit helling en punt','b uit helling<br>en punt',24,25,{playable:true}),
  node('equation_from_point_slope','Voorschrift uit helling en punt','Voorschrift uit<br>helling en punt',50,25,{playable:true}),
  node('equation_from_two_points','Voorschrift uit twee punten','Voorschrift uit<br>twee punten',76,24,{playable:true}),
 ...
 grenspas:{name:'Grenspas',caption:'Nulwaarden en tekens',intro:'Vind de grens. Ontdek het juiste gebied.',art:'grenspas',zones:[{id:'route',name:'De grensroute',nodes:[
  node('zeroRead','Nulwaarde aflezen','Nulwaarde<br>aflezen',28,24,{playable:true}),
  node('zero','Nulwaarde berekenen','Nulwaarde<br>berekenen',28,70,{playable:true}),
  node('sign','Wanneer is f(x) > 0?','Wanneer is<br>f(x) &gt; 0?',51,47,{key:'positive',variant:'positive',playable:true}),
  node('sign','Wanneer is f(x) < 0?','Wanneer is<br>f(x) &lt; 0?',75,25,{key:'negative',variant:'negative',playable:true}),
  node('signchart','Tekenschema','Tekenschema',82,70,{playable:true})
 ]}]}
};
...
function statuses(state,id,legacy){
 ...
 const formulaDone=n=>id==='formulewerf'&&n.playable&&(!!state.missions?.[n.id]?.completed||!!state.events?.some(e=>e.correct&&e.skill===n.id&&e.taskId?.startsWith('rechten-v2:formulewerf:'+n.id+':')&&/:5:run\d+$/.test(e.taskId)&&e.attemptId?.startsWith(({equation_from_ab:'formula-build:',graph_from_equation:'formula-plot:',equation_from_graph:'formula-read:',rewrite_linear_equation:'formula-rewrite:',intercept_from_point:'derive-intercept:',equation_from_point_slope:'derive-formula:',equation_from_two_points:'derive-formula:'})[n.id])));
