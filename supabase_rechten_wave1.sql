...
-- Apply before publishing Wave 1. Not applied by the browser or by this branch.
-- Keeps account authorization and revision checks; prevents v700 downgrade writes.
...
begin
  ...
  if p_revision is null or p_revision<0 or p_state is null or jsonb_typeof(p_state)<>'object'
    or octet_length(p_state::text)>262144 or coalesce(p_state->>'version','') not in ('700','701')
  ...
  if existing.state->>'version'='701' and p_state->>'version'='700' then
    raise exception 'Werk de Rechtentrainer bij: deze voortgang gebruikt Wave 1.' using errcode='22023';
  end if;
