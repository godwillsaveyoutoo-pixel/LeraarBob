...
-- Apply before publishing Wave 3 (includes the Wave 1 and 2 changes). Not applied by the browser or by this branch.
-- Keeps account authorization and revision checks; prevents v700/v701/v702 downgrade writes.
...
begin
  ...
  if p_revision is null or p_revision<0 or p_state is null or jsonb_typeof(p_state)<>'object'
    or octet_length(p_state::text)>262144 or coalesce(p_state->>'version','') not in ('700','701','702','703')
  ...
  if p_state->>'version' in ('701','702','703') then
  ...
  if p_state->>'version' in ('702','703') then
  ...
  if existing.state->>'version' in ('701','702','703') and (existing.state->>'version')::int>(p_state->>'version')::int then
