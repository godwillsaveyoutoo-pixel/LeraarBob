...
(async()=>{
 ...
 try{
  ...
  await go('#grenspas');await c.tap('[data-node="positive"]',true);assert.equal(await c.eval('document.querySelector("#app").dataset.screen'),'mission');await c.fill('root','3',true);const draft=(await snap()).missions.grenspas;await c.tap('#pause',true);assert.equal(await c.eval('location.hash'),'#grenspas');await c.tap('.atlas-footer [data-screen="world"]',true);await c.tap('[data-world-node="puntenbaai"]',true);await c.tap('[data-node="point"]',true);assert.deepEqual((await snap()).missions.grenspas,draft,'other areas preserve the working exercise');
  await go('#grenspas');await c.tap('[data-node="positive"]',true);assert.deepEqual((await snap()).missions.grenspas,draft);
