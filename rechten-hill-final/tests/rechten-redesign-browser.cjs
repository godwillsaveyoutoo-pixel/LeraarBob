...
(async()=>{const c=new CDP();await c.connect();
 ...
 async function actionLayout(){assert.deepEqual(await c.eval(`(()=>{
  const issues=[],footer=document.querySelector('.playFooter'),bounds=footer.getBoundingClientRect(),buttons=[...footer.querySelectorAll('button')].filter(b=>b.getClientRects().length),main=buttons.filter(b=>!b.dataset.footerAction),secondary=buttons.filter(b=>b.dataset.footerAction);
  if(main.length>1)issues.push('multiple confirmation buttons');
  for(const b of main){const r=b.getBoundingClientRect();if(Math.abs(r.right-bounds.right)>1)issues.push('confirmation is not at the right edge');for(const s of secondary)if(r.left-s.getBoundingClientRect().right<72)issues.push('secondary action too close to confirmation')}
  for(const b of secondary)if(b.getBoundingClientRect().right>innerWidth/2)issues.push('secondary action in the confirmation corner');
  const pause=document.querySelector('#pauseExercise');if(!pause.closest('.topbar'))issues.push('pause in exercise footer');
  for(const b of document.querySelectorAll('.topbar button')){if(!b.getClientRects().length)continue;const r=b.getBoundingClientRect();if(r.left<0||r.right>innerWidth+.5)issues.push('navigation clipped: '+b.textContent)}
  return issues;
 })()`),[])}
 async function layout(){await actionLayout();assert.deepEqual(await c.eval(`(()=>{const bad=[],els=[...document.querySelectorAll('#stage button,.concept-context,.concept-graph,.concept-givens,.concept-note,.coordinate-builder,.wave-context,.wave-equation,.wave-entry,.construct-formula,.transfer-story,.transfer-table,.algebra-previous,.equation-cue')].filter(e=>e.getClientRects().length);for(const e of els){const r=e.getBoundingClientRect(),play=document.querySelector('#stage .play').getBoundingClientRect();if(!e.closest('.playFooter')&&(r.top<play.top-1||r.bottom>play.bottom+1))bad.push('outside play '+e.className);if(r.left<-.5||r.right>innerWidth+.5||r.top<47||r.bottom>innerHeight+.5)bad.push(e.className+' clipped '+JSON.stringify(r.toJSON()));if(e.tagName==='BUTTON'&&(r.height<47.5||r.width<47.5))bad.push('small '+e.textContent)}const bs=els.filter(e=>e.tagName==='BUTTON');for(let i=0;i<bs.length;i++)for(let j=i+1;j<bs.length;j++){const a=bs[i].getBoundingClientRect(),b=bs[j].getBoundingClientRect();if(a.left<b.right-.5&&b.left<a.right-.5&&a.top<b.bottom-.5&&b.top<a.bottom-.5)bad.push('overlap '+bs[i].textContent+'/'+bs[j].textContent)}return bad})()`),[],await c.run('current.skill+":"+current.work.index'))}
 ...
 for(const [width,height,touch] of [[1366,768,false],[640,360,true]]){
  ...
  const beforePause=await c.run('JSON.stringify(current.work)');await tap('#pauseExercise',touch);assert.equal(await c.run('currentScreen'),'pause');await tap('[data-pause-resume]',touch);assert.equal(await c.run('JSON.stringify(current.work)'),beforePause);
