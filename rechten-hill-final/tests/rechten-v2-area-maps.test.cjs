...
test('all 27 original skills occur once, with precisely two separate sign variants and no merged skills',()=>{
 ...
 assert.deepEqual(nodes.filter(n=>n.id==='sign').map(n=>n.variant),['positive','negative']);assert.deepEqual(nodes.filter(n=>n.playable).map(n=>n.key),['point','point_plot','delta','slope','slope_from_two_points','line_behavior','special_lines','positive']);
...
test('a partial sign step never completes either sign stop; positive completion cannot complete the negative variant',()=>{
 const state=R.initial();state.events.push({skill:'sign',correct:true,variant:0,attemptId:'interval:2',phase:'execute'});assert.equal(A.statuses(state,'grenspas').nodes.find(n=>n.key==='positive').state,'current');
 state.events.push({skill:'sign',correct:true,variant:0,attemptId:'symbol:3',phase:'execute'});const status=A.statuses(state,'grenspas');assert.equal(status.nodes.find(n=>n.key==='positive').state,'completed');assert.equal(status.nodes.find(n=>n.key==='negative').state,'locked');
});
