...
test('the graph entry contract explicitly forbids hidden advanced requirements; the stop stays a preview',()=>{
...
test('Formulewerf recommendations are available and follow route order, never a locked fallback',()=>{
 ...
 const none=A.statuses(state,'formulewerf',legacy());assert.equal(none.recommended,null);assert(none.nodes.every(n=>n.state==='locked'));
 const html=S.area(A.locationState(state,'area',{area:'formulewerf'}),{legacy:legacy()});assert(html.includes('disabled>Geen halte beschikbaar'));assert(!html.includes('aria-current="step"'));
...
test('the zone footer points to the available next stop across A/B and never highlights a locked local stop',()=>{
 ...
 const a=A.locationState(R.initial(),'area',{area:'formulewerf',zone:'bouwen'}),raw=legacy(order.slice(0,8),order.slice(0,4));html=S.area(a,{legacy:raw});assert(!html.includes('aria-current="step"'));assert(html.includes('Naar werkplaats B'));assert(S.area(b,{legacy:raw}).includes('Volgende: halte 5'));
});
...
test('other routes, initial world/header, original validators, skills, scheduler, storage and art are preserved',()=>{
 ...
 const other=JSON.parse(JSON.stringify(Object.fromEntries(Object.entries(A.areas).filter(([id])=>id!=='formulewerf'))));for(const id of ['puntenbaai','hellingrug'])for(const n of other[id].zones[0].nodes)delete n.playable;assert.equal(hash(JSON.stringify(other)),manifest.otherAreas);
 for(const [id,expected] of Object.entries(manifest.views).filter(([id])=>!['puntenbaai','hellingrug'].includes(id)))assert.equal(hash(S.area(A.locationState(R.initial(),'area',{area:id}),{})),expected,id);
